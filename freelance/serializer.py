...
class JobsSerializer(serializers.ModelSerializer):
    ...
    def get_user_full_name(self, obj):
        ...
        image_url = [f"{user.image_url}" for user in users]
        return full_names

    class Meta:
        model = Job
        fields = ['job_id','jop_title','descriotion', 'user_full_name','image_url','min_price','max_price','entry_date','major_rel', 'major_name', 'image_url']

class JobSerializer(serializers.ModelSerializer):
    user_full_name = serializers.SerializerMethodField()

    def get_user_full_name(self, obj):
        users = obj.user_to_jop.all()  
        return users

    class Meta:
        model = Job
        fields = ['job_id','jop_title','descriotion', 'users',  'image_url','min_price','max_price','entry_date','major_name' ]
